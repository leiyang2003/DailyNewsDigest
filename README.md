...
运行 Flask 后，在浏览器打开 **http://127.0.0.1:5003/** 进入首页，从首页可进入以下模块：
...
会额外生成 `reports/podcast_YYYY-MM-DD_sync.json`。启动网页应用（`python app.py`）后，从首页点击「播客音频/同步朗读」或访问 `http://127.0.0.1:5003/sync_reader.html?date=2026-02-02`（将日期改为你生成的日期）。页面会加载该日期的 mp3 与 sync JSON，播放时按句高亮当前句；点击某句可跳转到该句播放。
...
从首页（http://127.0.0.1:5003/）点击「日语要点抽取」，或直接访问 `http://127.0.0.1:5003/japanese_points.html`。
